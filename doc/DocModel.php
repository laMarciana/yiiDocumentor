...
class DocModel
{
   ...
   protected function processClasses()
   {
      ...
      foreach($this->classes as $class=>$value)
      {
         ...
         /*Generic 'extensions' package for extensions which haven't any @package doc*/
         if (!$doc->package) {
            if ($doc->sourcePathType === 'ext') {
               $sourcePathArray = preg_split('/\//', $doc->sourcePath);
               $doc->package = 'extensions.'.strtolower($sourcePathArray[2]);
            } elseif ($doc->sourcePathType === 'current-ext') {
               $basePathArray = preg_split('/\//', $doc->basePath);
               $doc->package = 'extensions.'.strtolower($basePathArray[count($basePathArray) -1]);
            } elseif ($doc->sourcePathType === 'app') {
               $doc->package = 'application';
            }
         }
   ...
   protected function tagReturn($object,$comment)
   {
      ...
      if($object instanceof FunctionDocDoc)
      {
         $object->output=new ParamDocDoc($this->withYii, $this->basePath);
      ...
      else if($object instanceof PropertyDocDoc)
      {
         $object->type=$segs[0];
...
class BaseDocDoc
{
    ...
    * @var boolean whether to include yii framework class reference when documenting an aplication or an extension
    */
   public function __construct($withYii, $basePath) {
   ...
   public function loadSource($reflection)
   {
      ...
      if (substr($filename,0,strlen(YII_PATH)) === YII_PATH)
         $this->sourcePathType = 'yii';
      elseif (substr($filename,0,strlen(EXTENSIONS_PATH)) === EXTENSIONS_PATH) {
         if (substr($this->basePath,0,strlen(EXTENSIONS_PATH)) === EXTENSIONS_PATH)
            $this->sourcePathType = 'current-ext';
         elseif ($this->basePath === APP_PATH)
            $this->sourcePathType = 'ext';
      }
      elseif (substr($filename,0,strlen(APP_PATH)) === APP_PATH)
         $this->sourcePathType = 'app';
      if ($this->withYii && $this->sourcePathType === 'yii') {
         $this->sourcePath=str_replace('\\','/',str_replace(YII_PATH,'',$filename));
      } else {
         $this->sourcePath=str_replace('\\','/',str_replace($this->basePath,'',$filename));
      }
      if (!($reflection instanceof ReflectionProperty)) {
   ...
   public function getSourceCode()
   {
      if ($this->withYii) {
         if (file_exists($this->basePath.$this->sourcePath)) {
            $lines=file($this->basePath.$this->sourcePath);
         }
         else {
